...
class KeyLevelIdentifierV2_5:
    ...
    def identify_and_score_key_levels(self, 
        ...
        supports: List[KeyLevelV2_5] = []
        resistances: List[KeyLevelV2_5] = []
        
        if df_strike.empty:
            self.logger.warning("Strike DataFrame is empty, no key levels to identify.")
            return KeyLevelsDataV2_5(supports=[], resistances=[])
        ...
        if 'a_mspi_strike' in df_strike.columns:
            ...
            for peak_idx in peaks:
                strike = df_strike.loc[peak_idx, 'strike']
                if strike > current_price: # Only consider as resistance if above current price
                    potential_levels.append({'level': strike, 'type': 'AMSPI_Resistance', 'score': a_mspi_series.loc[peak_idx], 'source': 'a_mspi'})
            ...
            for trough_idx in troughs:
                strike = df_strike.loc[trough_idx, 'strike']
                if strike < current_price: # Only consider as support if below current price
                    potential_levels.append({'level': strike, 'type': 'AMSPI_Support', 'score': -a_mspi_series.loc[trough_idx], 'source': 'a_mspi'})
        ...
        supports_list: List[KeyLevel] = []
        resistances_list: List[KeyLevel] = []
        ...
        return KeyLevelsDataV2_5(supports=supports_list, resistances=resistances_list)
