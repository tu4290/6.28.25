...
from data_models.configuration_schemas import (
    EOTSConfigV2_5, 
...
def build_orchestrator(enhanced_cache):
    ...
    # Initialize market regime engine with config manager, elite_config, and enhanced_cache
    market_regime_engine = MarketRegimeEngineV2_5(
        config_manager, 
        elite_config,
        enhanced_cache=enhanced_cache
    )
    
    return market_regime_engine
...
def main():
    config_manager = ConfigManagerV2_5()
    elite_config: EliteConfig = config_manager.config.elite_config
            ...
            if is_market_open():
                for symbol in watched_tickers:
                    try:
                        ...
                        import asyncio
                        bundle = asyncio.run(market_regime_engine.run_full_analysis_cycle(symbol))
